...
public class TestDuelingTree {
	...
	public void testTwoSeparateRoundsEachSideWinsOnce() throws UnsupportedEncodingException {
		...
		final String expectedRoundWin = String.format("sounds/beep.wav%n").replace("/", File.separator) +
				String.format("left score: 0%n") +
				String.format("right score: 1%n");
		final String actualRoundWin = stringOut.toString("UTF-8");
		final String beepLine = String.format("sounds/beep.wav%n").replace("/", File.separator);
		String normalizedRoundWin = actualRoundWin;

		while (normalizedRoundWin.startsWith(beepLine)) {
			normalizedRoundWin = normalizedRoundWin.substring(beepLine.length());
		}

		while (normalizedRoundWin.endsWith(beepLine)) {
			normalizedRoundWin = normalizedRoundWin.substring(0, normalizedRoundWin.length() - beepLine.length());
		}

		assertEquals(expectedRoundWin, beepLine + normalizedRoundWin);
		stringOut.reset();
